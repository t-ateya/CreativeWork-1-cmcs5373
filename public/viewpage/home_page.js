...
//Here, we implement the page display information
    ...
    html = `
        ...
        You can reply to threads, create new threads and delete threads <br />
        However, you can only delete a thread you created. Nothing more!!! <br />
        Please, click on "new thread" to create a thread and "view" to reply to a thread <br />
        </h4>
...
function buildThreadView(thread) {
    ...
    `  
        ...
        return `        
            ...
            <td>
            <button disabled class="btn btn-outline-warning" data-toggle="modal" data-target="#${Constant.iDmodalDeleteForm}">Delete</button>
            </td>                 
            <td>${thread.title}</td>
            <td>${thread.keywordsArray.join(' ')}</td>
            <td>${thread.email}</td>
